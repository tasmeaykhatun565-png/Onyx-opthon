  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-4 border-b border-border-color">
        <button onClick={onBack} className="text-text-primary hover:bg-bg-tertiary p-1 rounded-full transition">
          <ChevronLeft size={28} />
        </button>
        <h1 className="text-xl font-bold text-text-primary">{t('settings.personal_info')}</h1>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-5 space-y-3">
            <div className="flex items-center gap-2 text-blue-500">
              <Gift size={20} />
              <h3 className="font-bold">{t('settings.referral_link')}</h3>
            </div>
            <p className="text-xs text-text-secondary">Share this link with your friends and earn bonuses on their first deposits.</p>
            <div className="flex items-center gap-2 bg-bg-primary p-3 rounded-xl border border-border-color">
              <input 
                type="text" 
                readOnly 
                value={referralLink}
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.fullname')}</label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-bg-secondary text-text-primary pl-10 pr-4 py-3 rounded-xl border border-border-color focus:outline-none focus:border-blue-500 transition"
              />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.dob')}</label>
            <div className="relative">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.gender')}</label>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    gender === g 
                      ...
                      : "bg-bg-secondary border-border-color text-text-secondary hover:bg-bg-secondary"
                  )}
                >
                  {g}
                </button>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.country')}</label>
            <div className="relative">
              <Globe className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={18} />
              <select 
                value={country}
                onChange={(e) => setCountry(e.target.value)}
                className="w-full bg-bg-secondary text-text-primary pl-10 pr-4 py-3 rounded-xl border border-border-color focus:outline-none focus:border-blue-500 transition appearance-none"
              >
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.timezone')}</label>
            <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
              <button 
                onClick={() => setIsTimezoneOpen(!isTimezoneOpen)}
                className="w-full p-3 flex items-center justify-between hover:bg-bg-tertiary transition"
              >
                <div className="flex items-center gap-3">
                  <Clock size={18} className="text-gray-500" />
                  <span className="font-medium text-text-primary">{selectedTimezone.label}</span>
                </div>
                <ChevronLeft size={20} className={cn("text-gray-500 transition-transform", isTimezoneOpen ? "-rotate-90" : "rotate-180")} />
              </button>
              
              {isTimezoneOpen && (
                <div className="border-t border-border-color max-h-60 overflow-y-auto">
                  {timezones.map((tz) => (
                    <button
                      key={tz.label}
                      onClick={() => {
                      ...
                      className={cn(
                        "w-full flex items-center justify-between p-3 px-4 transition border-b border-border-color last:border-0",
                        timezoneOffset === tz.offset 
                          ? "bg-blue-500/10" 
                          : "hover:bg-bg-tertiary"
                      )}
                    >
                      <span className="text-sm text-text-primary">{tz.label}</span>
                      {timezoneOffset === tz.offset && (
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.address')}</label>
            <div className="relative">
              ...
              <textarea 
                ...
                rows={3}
                className="w-full bg-bg-secondary text-text-primary pl-10 pr-4 py-3 rounded-xl border border-border-color focus:outline-none focus:border-blue-500 transition resize-none"
          ...
          {/* Currency Selection */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-text-secondary uppercase">{t('settings.currency')}</label>
            <div className="relative">
              <button 
                onClick={() => setIsCurrencyOpen(!isCurrencyOpen)}
                className="w-full bg-bg-secondary text-text-primary px-4 py-3 rounded-xl border border-border-color flex items-center justify-between hover:border-blue-500/50 transition"
              >
                <div className="flex items-center gap-3">
                  <span className="text-xl">{currency.flag}</span>
                  <div className="text-left">
                    <p className="text-sm font-bold">{currency.name}</p>
                    <p className="text-[10px] text-text-secondary uppercase font-bold">{currency.code} ({currency.symbol})</p>
                  </div>
                </div>
              ...
              {isCurrencyOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-bg-secondary border border-border-color rounded-xl overflow-hidden z-50 shadow-2xl max-h-[300px] overflow-y-auto">
                  {CURRENCIES.map(c => (
                    <button
                      key={c.code}
                      onClick={() => {
                      ...
                      className={cn(
                        "w-full px-4 py-3 flex items-center justify-between hover:bg-bg-tertiary transition",
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[10px] text-text-secondary uppercase font-bold">{c.code}</p>
                        </div>
                      </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                value={email}
                readOnly
                className="w-full bg-bg-secondary text-text-secondary pl-10 pr-12 py-3 rounded-xl border border-border-color focus:outline-none cursor-not-allowed opacity-70"
              />
              <div className="absolute right-3 top-1/2 -translate-y-1/2">
                <span className="text-xs font-bold text-green-500 bg-green-500/10 px-2 py-1 rounded">Verified</span>
              </div>
            </div>
            <p className="text-[10px] text-text-secondary">To change your email, please contact support.</p>
          </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-4 border-b border-border-color">
        ...
        <h1 className="text-xl font-bold text-text-primary">Identity Verification</h1>
      </div>
        ...
        {step === 'status' ? (
          <div className="space-y-6">
            ...
            <div className={cn(
              ...
              "bg-blue-500/5 border-border-color"
            )}>
              <div className={cn(
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="p-4 bg-bg-secondary rounded-2xl border border-border-color flex gap-4">
                <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-500 shrink-0">
                  <Shield size={20} />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-text-primary">Secure & Encrypted</h4>
                  <p className="text-xs text-text-secondary mt-1">Your documents are stored with bank-grade encryption and only used for verification.</p>
                </div>
              </div>
              <div className="p-4 bg-bg-secondary rounded-2xl border border-border-color flex gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-text-primary">Fast Processing</h4>
                  <p className="text-xs text-text-secondary mt-1">Our team reviews most submissions within 24 hours on business days.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6 pb-10">
                  ...
                  <React.Fragment key={s.id}>
                    <div className="flex flex-col items-center gap-2 relative">
                      ...
                      <span className={cn(
                        ...
                        isActive ? "text-blue-500" : isCompleted ? "text-green-500" : "text-text-secondary"
                      )}>{s.label}</span>
                    </div>
                    {i < 2 && (
                      <div className="flex-1 h-[2px] bg-[var(--color-border-color)] mx-2 -mt-6 relative overflow-hidden">
                        <div className={cn(
            ...
            {step === 'personal' && (
              ...
              >
                <div className="space-y-4">
                  <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest">Select Document Type</h3>
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest">Personal Details</h3>
                  <div className="space-y-3">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">Full Name (As on Document)</label>
                      <input 
                        type="text"
                        value={formData.fullName}
                        onChange={(e) => setFormData(prev => ({ ...prev, fullName: e.target.value }))}
                        placeholder="Enter your full name"
                        className="w-full bg-bg-secondary border border-border-color rounded-xl px-4 py-3 text-sm focus:border-blue-500 outline-none transition"
                      />
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">Date of Birth</label>
                      <input 
                        type="date"
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">Gender</label>
                ...
                <div className="flex gap-3 pt-4">
                  <button 
                    ...
                    className="flex-1 py-4 bg-bg-secondary border border-border-color text-text-primary font-black rounded-xl transition uppercase tracking-widest text-xs"
                  >
                    Back
                  </button>
                  <button 
            ...
            {step === 'documents' && (
              ...
              >
                <div className="bg-blue-500/5 border border-blue-500/20 rounded-2xl p-5 space-y-4">
                  ...
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={i} className="flex gap-3 text-[10px] text-text-secondary">
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest">Document Photos</h3>
                  <div className={cn("grid gap-4", documentType === 'NID' ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1")}>
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">
                        {documentType === 'NID' ? 'Front Side' : 'Main Page'}
                      </label>
                      <div className="relative aspect-[3/2] rounded-2xl border-2 border-dashed border-border-color bg-bg-secondary overflow-hidden group hover:border-blue-500/50 transition">
                        {formData.frontImage ? (
                    ...
                    {documentType === 'NID' && (
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">Back Side</label>
                        <div className="relative aspect-[3/2] rounded-2xl border-2 border-dashed border-border-color bg-bg-secondary overflow-hidden group hover:border-blue-500/50 transition">
                          {formData.backImage ? (
            ...
            {step === 'selfie' && (
              ...
              >
                ...
                <div className="space-y-4">
                  <h3 className="text-xs font-black text-text-secondary uppercase tracking-widest">Security Selfie</h3>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-text-secondary uppercase ml-1">Selfie with Document</label>
                    <div className="relative aspect-[16/9] rounded-2xl border-2 border-dashed border-border-color bg-bg-secondary overflow-hidden group hover:border-blue-500/50 transition">
                ...
                <div className="pt-2">
                  ...
                  >
                    <div className={cn(
                      ...
                      agreedToTerms ? "bg-blue-600 border-blue-500 text-white" : "bg-bg-secondary border-border-color"
                    )}>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button 
                    ...
                    className={cn(
                      ...
                      isSubmitting 
                        ? "bg-blue-600/50 text-text-secondary/50 cursor-not-allowed" 
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-border-color">
        ...
        <h1 className="text-[17px] font-bold text-text-primary absolute left-1/2 -translate-x-1/2">{t('settings.notifications')}</h1>
        <button onClick={onBack} className="text-text-secondary/70 hover:text-text-primary transition">
          <X size={28} />
        </button>
      </div>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="space-y-2">
            <div className="relative">
              <input 
                ...
                className="w-full bg-bg-secondary text-text-primary pl-4 pr-12 py-4 rounded-xl border border-transparent focus:border-border-color outline-none transition"
                placeholder="Current password"
              />
              <button 
                onClick={() => setShowCurrent(!showCurrent)}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-text-secondary/50 hover:text-text-primary"
              >
        ...
        <button 
          ...
          className="w-full bg-[#4ade80] hover:bg-[#22c55e] text-black font-bold py-4 rounded-xl transition active:scale-[0.98]"
        >
          Change Password
        </button>
      </div>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        <div className="flex flex-col items-center text-center py-8">
          ...
          <h2 className="text-xl font-bold text-text-primary">Protect Your Account</h2>
        ...
        <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
          <div className="p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-text-primary">Authenticator App</h3>
                <p className="text-[10px] text-text-secondary">Use Google Authenticator or Authy</p>
              </div>
            </div>
        ...
        {isSettingUp && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
            <div className="p-4 bg-bg-secondary rounded-xl border border-border-color flex flex-col items-center gap-4">
              {qrCodeUrl ? (
                <div className="w-48 h-48 bg-white p-2 rounded-lg">
                  <img src={qrCodeUrl} alt="2FA QR Code" className="w-full h-full" />
                </div>
              ) : (
                <div className="w-48 h-48 bg-bg-secondary animate-pulse rounded-lg" />
              )}
              <div className="text-center">
                <p className="text-xs text-text-secondary mb-2">Scan this QR code with your authenticator app</p>
                <div className="bg-black/30 px-4 py-2 rounded-lg font-mono text-sm tracking-wider flex items-center gap-2">
                  ...
                  <button onClick={() => {
                    ...
                    <Copy size={14} className="text-gray-500 cursor-pointer hover:text-text-primary" />
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-text-secondary uppercase">Enter Code</label>
              <input 
                type="text" 
                value={code}
                onChange={(e) => setCode(e.target.value)}
                placeholder="000 000"
                className="w-full bg-bg-secondary text-text-primary text-center text-xl tracking-widest py-3 rounded-xl border border-border-color focus:outline-none focus:border-blue-500 transition"
                maxLength={6}
              />
            </div>
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 bg-bg-tertiary hover:bg-bg-tertiary text-text-primary font-bold py-4 rounded-xl transition"
              >
                Cancel
              </button>
              <button 
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                onChange={(e) => setCode(e.target.value)}
                placeholder="000 000"
                className="w-full bg-bg-secondary text-text-primary text-center text-xl tracking-widest py-3 rounded-xl border border-border-color focus:outline-none focus:border-red-500 transition"
                maxLength={6}
              />
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-4 border-b border-border-color">
        ...
        <h1 className="text-xl font-bold text-text-primary">App PIN</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {enabled && localStorage.getItem('pin-enabled') !== 'true' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="text-center">
              <h3 className="text-lg font-bold text-text-primary">Set Your PIN</h3>
              <p className="text-xs text-text-secondary">Enter a 4-digit PIN code</p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-xs mx-auto">
              {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                <button
                  ...
                  className="h-16 rounded-xl bg-bg-secondary hover:bg-bg-secondary border border-border-color text-2xl font-bold text-text-primary transition active:scale-95"
                >
                  {num}
                </button>
              ))}
              ...
              <button
                ...
                className="h-16 rounded-xl bg-bg-secondary hover:bg-bg-secondary border border-border-color flex items-center justify-center text-text-primary transition active:scale-95"
              >
                <ChevronLeft size={24} />
              </button>
            </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-4 border-b border-border-color">
        ...
        <h1 className="text-xl font-bold text-text-primary">{t('nav.terminal')}</h1>
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Trading Mode</h2>
          <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
            <ToggleItem 
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Performance</h2>
          <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
            <ToggleItem 
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4 p-4 border-b border-border-color">
        ...
        <h1 className="text-xl font-bold text-text-primary">{t('settings.appearance')}</h1>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Currency</h2>
          <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
            <button 
              onClick={() => setIsCurrencyOpen(!isCurrencyOpen)}
              className="w-full p-4 flex items-center justify-between hover:bg-bg-tertiary transition"
            >
              <div className="flex items-center gap-3">
                <span className="text-2xl">{currency.flag}</span>
                <div className="flex flex-col items-start">
                  <span className="font-bold text-sm text-text-primary">{currency.code}</span>
                  <span className="text-[10px] text-text-secondary">{currency.name}</span>
                </div>
              </div>
                ...
                {CURRENCIES.map((c) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{c.flag}</span>
                      <div className="flex flex-col items-start">
                        <span className="font-bold text-sm text-text-primary">{c.code}</span>
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Timezone (Chart Time)</h2>
          <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
            <button 
              onClick={() => setIsTimezoneOpen(!isTimezoneOpen)}
              className="w-full p-4 flex items-center justify-between hover:bg-bg-tertiary transition"
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-text-primary">{selectedTimezone.label}</span>
              </div>
              <ChevronLeft size={20} className={cn("text-gray-500 transition-transform", isTimezoneOpen ? "-rotate-90" : "rotate-180")} />
            </button>
            ...
            {isTimezoneOpen && (
              <div className="border-t border-border-color max-h-60 overflow-y-auto">
                {timezones.map((tz) => (
                  ...
                  >
                    <span className="text-sm font-medium text-text-primary">{tz.label}</span>
                    {timezoneOffset === tz.offset && (
                      <Check size={16} className="text-blue-500" strokeWidth={3} />
                    )}
                  </button>
        ...
        <section className="space-y-4">
          <h2 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Sound Effects</h2>
          <div className="bg-bg-secondary rounded-xl overflow-hidden border border-border-color">
            <ToggleItem 
...
const ToggleItem = ({ 
  ...
  <div className={cn(
    "p-4 flex items-center justify-between cursor-pointer transition active:scale-[0.99]",
    ...
    <div className="flex items-center gap-4 overflow-hidden">
      ...
      <div className="overflow-hidden">
        <h3 className="text-[15px] font-medium text-text-primary leading-tight truncate">{label}</h3>
        <p className="text-[13px] text-text-secondary/40 leading-snug mt-0.5 line-clamp-2">{description}</p>
      </div>
    </div>
    <div className={cn(
      "w-11 h-6 rounded-full relative shrink-0 transition-colors duration-200",
      enabled ? "bg-[#00e676]" : "bg-bg-tertiary"
    )}>
      <div className={cn(
...
const ThemeButton = ({ active, onClick, icon, label, color }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string, color: string }) => (
  <button 
    ...
    className={cn(
      ...
      active ? "border-blue-500 ring-1 ring-blue-500" : "border-border-color bg-bg-secondary"
    )}
  >
    <div className="absolute top-0 left-0 w-full h-1" style={{ backgroundColor: color }} />
    <div className={cn(
      "p-2 rounded-full",
      active ? "bg-blue-500/20 text-blue-500" : "bg-bg-secondary text-text-secondary"
    )}>
      {icon}
    </div>
    <span className={cn(
      "text-[10px] font-bold uppercase tracking-wider",
      active ? "text-blue-500" : "text-text-secondary"
    )}>{label}</span>
    {active && (
